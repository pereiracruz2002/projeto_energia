...
var RelatorioClass = function ()
{
    ...
    this.getData = function ()
    {
        ...
        $.post(base_url + 'relatorios/banners', $('.page-header input, .page-header select').serialize(), function (result) {
            elm.find('.loading').remove();
            elm.find('.reportContent').fadeIn();

            var tbody = '';
            $.each(result, function(key, val){
                tbody += '<tr><td>'+val.nome+'</td><td>'+val.posicao+'</td><td>'+val.views+'</td><td>'+val.clicks+'</td></tr>';
            })
